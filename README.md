...
- FastAPIを用いてREST APIとしてタスク操作を実装した
- SQLAlchemyを使ってPostgreSQLと接続した
- フロントエンドから非同期通信でAPIを呼び出す構成にした
- pytestを用いてCRUDの動作確認を行えるようにした
- テストDBを分けることで、本番用データを壊さず検証できるようにした
